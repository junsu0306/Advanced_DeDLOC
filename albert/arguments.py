...
class AveragerArguments:
    ...
    # Adaptive Averaging을 위한 추가 항목
    use_adaptive_averaging: bool = field(
        default=False,
        metadata={"help": "If True, use adaptive averaging strategy based on LP optimization to dynamically assign roles in gradient aggregation."}
    )
    lp_solver_timeout: float = field(
        default=0.05,
        metadata={"help": "Maximum time (in seconds) allowed for the LP solver to compute the optimal strategy."}
    )
    max_lp_iterations: int = field(
        default=50,
        metadata={"help": "Maximum number of iterations for the LP solver."}
    )
