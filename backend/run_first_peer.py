...
from transformers import BertForMaskedLM, BertConfig, HfArgumentParser
import wandb
...
class CoordinatorArguments(BaseTrainingArguments):
    address: Optional[str] = field(
        default=None,
        metadata={"help": "This machine's network address. Use public IP for global experiments, local address for private runs"},
    )
...
class CheckpointHandler:
    ...
    def is_time_to_save_state(self, cur_step):
        return self.save_checkpoint_step_interval and (cur_step - self.previous_step >= self.save_checkpoint_step_interval)
    ...
    def is_time_to_upload(self):
        return self.repo_path and (time.time() - self.previous_timestamp >= self.upload_interval)
...
if __name__ == "__main__":
    ...
    # ✅ 이 줄 추가! /p2p/... 주소 출력
    logger.info(f"Visible address: {dht.get_visible_address()}")
    ...
    while True:
        ...
        if metrics_dict is not None:
            ...
            if latest_step != current_step:
                ...
                sum_loss = sum(item.loss for item in metrics)
                sum_steps = sum(item.mini_steps for item in metrics)
                alive_peers = len(metrics)
                total_samples = sum(item.samples_accumulated for item in metrics)
                total_perf = sum(item.samples_per_second for item in metrics)
                current_loss = sum_loss / sum_steps if sum_steps > 0 else 0.0
